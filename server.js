...
app.get("/api/home", async (req, res) => {
  try {
    const v = await q(`SELECT
      ...
      (SELECT count(*) FROM video_likes l WHERE l.video_id::text=v.id::text) AS likes
      FROM videos v JOIN channels c ON c.id::text=v.channel_id::text
...
app.get("/api/channels/:id", async (req, res) => {
  try {
    ...
    const v = await q(`SELECT v.*,
      (SELECT count(*) FROM video_likes l WHERE l.video_id::text=v.id::text) AS likes
      FROM videos v WHERE v.channel_id::text=$1::text ORDER BY v.created_at DESC`, [req.params.id]);
...
app.get("/api/videos/:id", async (req, res) => {
  ...
  try {
    const r = await q(`SELECT v.*, c.name AS channel_name, c.avatar_url AS channel_avatar,
      (SELECT count(*) FROM video_likes l WHERE l.video_id::text=v.id::text) AS likes,
      EXISTS(SELECT 1 FROM video_likes l2 WHERE l2.video_id::text=v.id::text AND l2.user_key=$2) AS liked,
      (c.owner_profile_id=$2) AS is_owner
      FROM videos v JOIN channels c ON c.id::text=v.channel_id::text WHERE v.id=$1`, [req.params.id, ident(req, res)]);
...
app.get("/api/owner/data", async (req, res) => {
  ...
  try {
    const [v, c] = await Promise.all([
      q(`SELECT v.*,c.name AS channel_name FROM videos v JOIN channels c ON c.id::text=v.channel_id::text ORDER BY v.created_at DESC`),
