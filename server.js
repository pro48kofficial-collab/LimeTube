...
async function ensureBucket(name) {
  ...
  if (error && !/not found|404/i.test(error.message || "")) {
    throw new Error(`Не можу перевірити bucket ${name}: ${error.message}`);
  }
  const { error: createError } = await supabase.storage.createBucket(name, {
    public: true
  });
  if (createError && !/already exists/i.test(createError.message || "")) {
    throw new Error(`Не можу створити bucket ${name}: ${createError.message}`);
  }
}
...
async function init() {
  ...
  // If the old database already contains several channels for one profile,
  // we intentionally do not delete anything. The API simply uses the newest one.
  await Promise.all([
...
app.post("/api/upload", videoUpload.fields([
  ...
  try {
    ...
    const msg = /File too large/i.test(e.message) ? `Файл завеликий. Максимум ${videoLimitMb} MB.` : e.message;
