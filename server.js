...
function validImageDataUrl(value, maxChars = 900000) {
  if (!value) return true;
  const s = String(value);
  return s.length <= maxChars && /^data:image\/(jpeg|jpg|png|webp);base64,/i.test(s);
}
...
app.post("/api/profile", async (req, res) => {
  try {
    ...
    if (!validImageDataUrl(avatarUrl)) return res.status(400).json({ error: "Некоректна аватарка" });
...
app.post("/api/channel", async (req, res) => {
  try {
    ...
    if (!validImageDataUrl(req.body.avatar_url) || !validImageDataUrl(req.body.banner_url)) return res.status(400).json({ error: "Некоректна аватарка або банер" });
