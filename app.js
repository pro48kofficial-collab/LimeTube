...
async function saveProfile(e) {
  ...
  try {
    ...
    if (file) avatar = await uploadImage(file, "avatar");
...
async function saveChannel(e) {
  ...
  try {
    ...
    if (af) avatar = await uploadImage(af, "avatar");
    if (bf) banner = await uploadImage(bf, "banner");
...
async function uploadImage(file, type) {
  ...
  if (!/^image\/(jpeg|png|webp)$/i.test(file.type)) {
    throw new Error("Вибери JPG, PNG або WebP");
  }
  if (file.size > 8 * 1024 * 1024) {
    throw new Error("Зображення завелике. Максимум 8 МБ.");
  }

  // Для аватарів/банерів НЕ ганяємо файл через canvas: на Android це могло
  // зависати або ламати вибране фото. Відправляємо оригінальний файл напряму.
  const fd = new FormData();
  fd.append("type", type);
  fd.append("image", file, file.name || `${type}-${Date.now()}.jpg`);
  const d = await api("/api/upload-image", { method:"POST", body:fd });
  if (!d.url) throw new Error("Supabase не повернув адресу зображення");
  return d.url;
}
